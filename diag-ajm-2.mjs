...
console.log("\n=== JOURNAL ENTRIES ===");
const jnls = (db.journalEntries || []).filter(j => j.description && (j.description.includes(memberId) || (member && j.description.includes(member.fullName))));
console.log(jnls);

console.log("\n=== MEMBER EXITS ===");
const exits = (db.memberExits || []).filter(a => a.memberId === memberId);
console.log(exits);
