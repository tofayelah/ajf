...
export const EarlyMemberExitView = () => {
  ...
  const exits = (db.memberExits || []).filter(e => e.exitType === 'EARLY');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        <div>
          <h2 className="text-base sm:text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            <span>{isBangla ? 'আগাম প্রস্থান' : 'Early Member Exit'}</span>
          </h2>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          ...
          >
            ...
            <option value="EARLY_EXIT_REQUESTED">Pending Request</option>
                ...
                return (
                  <tr key={realId} className="hover:bg-amber-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {member?.joiningDate || (member as any)?.admissionDate || 'N/A'}
                    </td>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="font-bold text-lg text-slate-900">
                {isBangla ? 'নতুন আগাম প্রস্থান' : 'New Early Exit'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500"
                >
