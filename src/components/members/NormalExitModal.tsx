...
import { X, AlertCircle, CheckCircle2, UserX, Wallet, FileText, Ban, ShieldCheck } from 'lucide-react';
import { Member, MemberExitRequest, PaymentMethod } from '../../types';
import { useApp } from '../../context/AppContext';
import { formatCurrency, formatDate } from '../../utils/helpers';

export const NormalExitModal: React.FC<{ member: Member; onClose: () => void }> = ({ member, onClose }) => {
  const { db, activeUser, requestMemberExit, reviewMemberExit, approveMemberExit, rejectMemberExit, processMemberExitRefund } = useApp();
  const existingRequest = db.memberExits?.find(e => e.memberId === member.memberId && e.exitType === "NORMAL" && e.status !== "REJECTED" && e.status !== "EXITED" && e.status !== "REFUNDED" && e.status !== "SETTLED");
  ...
  const isEligible = tenureYears >= 3;
  ...
  const handleRequest = async () => {
    setErrorMsg(null);
    if (!exitReason.trim()) return setErrorMsg("Exit reason is mandatory.");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl flex flex-col my-auto max-h-[95vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-blue-50/80 rounded-t-2xl">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center"><UserX className="w-5 h-5" /></div>
            <div>
              <h2 className="text-lg font-bold text-blue-900">Normal Member Exit</h2>
              <p className="text-sm text-blue-700">{member.fullName} ({member.memberId})</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 grid grid-cols-2 gap-4 text-sm mb-6">
            ...
            <div><p className="text-slate-500 mb-1">Membership Tenure</p><p className="font-bold text-slate-800">{tenureYears} Years {tenureMonths} Months</p></div>
          </div>

          {!existingRequest && !isEligible ? (
             <div className="py-8 text-center bg-rose-50 rounded-xl border border-rose-200">
               <Ban className="w-12 h-12 text-rose-500 mx-auto mb-3" />
               <p className="text-lg font-bold text-rose-800">৩ বছর পূর্ণ না হওয়ায় Normal Exit করা যাবে না।<br/>Early Exit ব্যবহার করুন।</p>
               <button onClick={onClose} className="mt-4 px-6 py-2 bg-white border hover:bg-slate-50 text-slate-700 font-medium rounded-lg">Close</button>
             </div>
          ) : existingRequest ? (
            <div className="space-y-6">
               <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                  <div className="bg-slate-50 px-4 py-3 border-b flex justify-between items-center">
                    ...
                    <span className="px-2.5 py-1 bg-blue-100 text-blue-700 rounded-md text-xs font-bold">{existingRequest.status}</span>
                  </div>
                  <div className="p-4 grid grid-cols-2 gap-4 text-sm">
                    <div className="col-span-2"><span className="text-slate-500">Reason:</span><p className="mt-1 p-3 bg-slate-50 rounded-lg text-slate-700">{existingRequest.exitReason}</p></div>
                  </div>
               </div>
               
               <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                  <h4 className="font-bold text-blue-900 mb-3 flex items-center gap-2"><Wallet className="w-4 h-4" /> Financial Settlement</h4>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between text-blue-800"><span>Eligible Capital</span><span className="font-semibold">{formatCurrency(existingRequest.eligibleRefundAmount)}</span></div>
                    ...
                    <div className="flex justify-between text-emerald-700 font-bold text-base pt-2 border-t border-blue-200"><span>Net Refund Amount</span><span>{formatCurrency(existingRequest.netRefundAmount)}</span></div>
                  </div>
               </div>

               {errorMsg && <div className="p-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg text-sm">{errorMsg}</div>}

               {existingRequest.status === 'NORMAL_EXIT_REQUESTED' && canApprove && (
               ...
               {existingRequest.status === 'UNDER_REVIEW' && canApprove && (
                  <div className="space-y-4 pt-4 border-t">
                    <div className="flex justify-end gap-3">
                      ...
                      <button onClick={() => handleApprove(existingRequest.exitRequestId)} className="px-5 py-2.5 bg-emerald-600 text-white rounded-xl flex items-center gap-2"><ShieldCheck className="w-4 h-4" /> Approve</button>
                    </div>
            ...
            <div className="space-y-6">
               <div className="bg-blue-50 border border-blue-200 p-5 rounded-xl space-y-4">
                 <h4 className="font-bold text-blue-900 flex items-center gap-2 border-b border-blue-100 pb-2"><Wallet className="w-4 h-4 text-blue-600" /> Financial Calculation</h4>
                 <div className="space-y-2 text-sm">
                   <div className="flex justify-between text-blue-800"><span>Eligible Capital</span><span className="font-semibold">{formatCurrency(eligibleRefundAmount)}</span></div>
                   ...
                   <div className="flex justify-between text-emerald-700 font-bold text-lg pt-2 border-t border-blue-200"><span>Net Refund Amount</span><span>{formatCurrency(netRefundAmount)}</span></div>
                 </div>
               </div>

               <div>
                 <label className="block text-sm font-bold text-slate-700 mb-2">Exit Reason *</label>
                 <textarea value={exitReason} onChange={(e) => setExitReason(e.target.value)} className="w-full border-slate-300 rounded-xl p-3 h-24 text-sm" placeholder="Reason for resignation..." />
               </div>
               ...
               <label className="flex items-start gap-3 p-4 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer">
                 ...
                 <span className="text-sm font-medium text-slate-700">I confirm the above information is correct and request normal member exit.</span>
               </label>
               {errorMsg && <div className="p-3 bg-rose-50 border border-rose-200 text-rose-700 rounded-lg text-sm">{errorMsg}</div>}
               <div className="flex justify-end gap-3 pt-4 border-t">
                 <button onClick={onClose} className="px-6 py-2.5 bg-white border text-slate-700 rounded-xl">Cancel</button>
                 <button onClick={handleRequest} disabled={!declarationChecked} className="px-6 py-2.5 bg-blue-600 text-white rounded-xl disabled:opacity-50">Submit Request</button>
               </div>
            </div>
          )}
        </div>
