...
const newWriteDbFile = `async function writeDbFile(db, options = {}) {
  ...
  if (isProduction && !options.skipSafetyCheck) {
    try {
      ...
      // If DB file doesn't exist or is invalid, we allow writing (it will be caught by init check if necessary)
    }
  }
...
/async function writeDbFile\(db\) \{[\s\S]*?await fs\.rename\(tempFile, DB_FILE\);\n\}/,
newWriteDbFile
);
